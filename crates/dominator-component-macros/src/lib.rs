...
fn get_type_generic_param_use(type_: &Type, struct_generics: &Vec<PropGenerics>) -> Vec<PropGenerics> {
    let mut out = vec![];

    match &type_ {
        Type::Path(type_path) => {
            for segment in &type_path.path.segments {
                if let Some(generic) = struct_generics.iter().find(|generic| generic.param.ident.to_string() == segment.ident.to_string()) {
...
pub fn component_attr(args: TokenStream, input: TokenStream) -> TokenStream {
    let struct_ = syn::parse::<syn::ItemStruct>(input.clone()).expect("failed to parse struct");
    ...
    let props = fields.iter().map(|field| {
        let is_signal = field.attrs.iter().any(|a| {
            a.path().is_ident("signal")
        });
